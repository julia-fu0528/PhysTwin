...
def preprocess_phystwin(cfg, dataset_root_episode, source_data_root_episode, dt_frame, dx, **kwargs):
    """Preprocess PhysTwin/ParticleFormer data format to PGND format.
    
    Reads from:
    - final_data.pkl: object_points, controller_points
    - robot/robot.npy: T_worlds, openings
    - split.json: train/test frame ranges
    
    Saves per-frame .pt files with:
    - x: particle positions (n_particles, 3)
    - grippers: gripper state (n_grippers, 15)
    ...
    if save:
        ...
        print('Preprocessing episode in-memory (PhysTwin format):', source_data_root_episode.name)
    ...
    controller_points = data.get('controller_points', None)  # (T, M, 3)
    
    # Load robot data if available
    robot_path = source_data_root_episode / 'robot' / 'robot.npy'
    if robot_path.exists():
        robot_data = np.load(robot_path, allow_pickle=True).item()
        T_worlds = robot_data['T_worlds']  # (T, 4, 4)
        openings = robot_data['openings']  # (T,)
        bimanual = robot_data.get('bimanual', False)
    else:
        T_worlds = None
        openings = None
        bimanual = False
    
    # Load split.json to apply frame range
    # Note: object_points in final_data.pkl is already offset to start at train[0]
    # But T_worlds/openings from robot.npy are from raw data
    split_path = source_data_root_episode / 'split.json'
    if split_path.exists():
        with open(split_path, 'r') as f:
            split_data = json.load(f)
        train_range = split_data.get('train', [0, object_points.shape[0]])
        test_range = split_data.get('test', train_range)
        raw_offset = train_range[0]  # Offset in raw data
        # Include frames from train start to test end (so eval can produce test metrics)
        total_end = max(train_range[1], test_range[1])
        total_len = total_end - raw_offset  # Number of frames from train start to test end
        
        # object_points is already aligned to start at train[0], so take first total_len frames
        if object_points.shape[0] > total_len:
            object_points = object_points[:total_len]
        if controller_points is not None and controller_points.shape[0] > total_len:
            controller_points = controller_points[:total_len]
        
        # T_worlds and openings need to be sliced from raw offset
        if T_worlds is not None:
            T_worlds = T_worlds[raw_offset:total_end]
    ...
    # Extract gripper positions from T_worlds or controller_points
    if T_worlds is not None:
        # Use robot transform for gripper position
        eef_xyz = T_worlds[:, :3, 3]  # (n_frames_robot, 3) - just translation
        eef_xyz = eef_xyz[:, np.newaxis, :]  # (n_frames_robot, 1, 3)
        
        # Extract rotation matrix
        eef_rot = T_worlds[:, :3, :3]  # (n_frames_robot, 3, 3)
        n_frames_robot = eef_rot.shape[0]
        eef_rot = eef_rot[:, np.newaxis, :, :]  # (n_frames_robot, 1, 3, 3)
        eef_rot = torch.from_numpy(eef_rot.astype(np.float32)).reshape(-1, 3, 3)
        eef_quat = kornia.geometry.conversions.rotation_matrix_to_quaternion(eef_rot).numpy()
        eef_quat = eef_quat.reshape(n_frames_robot, 1, 4)
        
        # Ensure we have the right number of grippers
        if num_grippers > 1:
            eef_xyz = np.tile(eef_xyz, (1, num_grippers, 1))
            eef_quat = np.tile(eef_quat, (1, num_grippers, 1))
    elif controller_points is not None:
        # Use mean of controller points as gripper position
        eef_xyz = controller_points.mean(axis=1, keepdims=True)  # (n_frames, 1, 3)
        eef_quat = np.zeros((n_frames, 1, 4), dtype=np.float32)
        eef_quat[:, :, 0] = 1.0  # identity quaternion
    ...
    # Compute gripper velocities
    eef_vel = np.zeros_like(eef_xyz)  # (n_frames, n_grippers, 3)
    eef_vel[:-1] = (eef_xyz[1:] - eef_xyz[:-1]) / dt_frame
    eef_vel[-1] = eef_vel[-2]
    
    # Compute rotational velocity
    eef_rot_this = kornia.geometry.conversions.quaternion_to_rotation_matrix(
        torch.from_numpy(eef_quat[:-1].astype(np.float32)).reshape(-1, 4))
    eef_rot_next = kornia.geometry.conversions.quaternion_to_rotation_matrix(
        torch.from_numpy(eef_quat[1:].astype(np.float32)).reshape(-1, 4))
    ...
    eef_quat_vel = np.zeros((n_frames, num_grippers, 3), dtype=np.float32)
    eef_quat_vel[:-1] = eef_aa.numpy().reshape(n_frames - 1, -1, 3) / dt_frame
    eef_quat_vel[-1] = eef_quat_vel[-2]
    
    # Gripper opening values
    if openings is not None:
        eef_gripper = openings[:, np.newaxis]  # (n_frames, 1)
        if num_grippers > 1:
            eef_gripper = np.tile(eef_gripper, (1, num_grippers))
    else:
        eef_gripper = np.zeros((n_frames, num_grippers), dtype=np.float32)
    
    # Build gripper array (15 values per gripper)
    grippers = np.zeros((n_frames, num_grippers, 15), dtype=np.float32)
    grippers[:, :, :3] = eef_xyz
    ...
    # Normalize to simulation bounds
    ...
    # Clip particles that are out of bounds
    xyz_out_of_bound = (
        (xyz[:, :, 0].min() < dx * (cfg.model.clip_bound + 0.5) - 1e-6) |
        (xyz[:, :, 0].max() > 1 - dx * (cfg.model.clip_bound + 0.5) + 1e-6) |
        (xyz[:, :, 1].min() < dx * (cfg.model.clip_bound + 0.5) - 1e-6) |
        (xyz[:, :, 1].max() > 1 - dx * (cfg.model.clip_bound + 0.5) + 1e-6) |
        (xyz[:, :, 2].min() < dx * (cfg.model.clip_bound + 0.5) - 1e-6) |
        (xyz[:, :, 2].max() > 1 - dx * (cfg.model.clip_bound + 0.5) + 1e-6)
    )
    
    if xyz_out_of_bound:
        print(dataset_root_episode.name, 'particles out of bound, filtering...')
    ...
    for frame_id in range(n_frames):
        ...
        if save:
            torch.save(frame_data, dataset_root_episode / f'{frame_id:04d}.pt')
        else:
            states_list.append(frame_data)
    
    if save:
        print(f'Saved {n_frames} frames with {xyz.shape[1]} particles')
        return True
    else:
        print(f'Processed {n_frames} frames in-memory')
        return True, states_list, info


# Keep original preprocess for backward compatibility
def preprocess(cfg, dataset_root_episode, source_data_root_episode, dt_frame, dx, **kwargs):
    """Original PGND preprocess (expects traj.npz format).
    
    Use preprocess_phystwin for PhysTwin/ParticleFormer data format.
    """
    # Check if this is PhysTwin format (has final_data.pkl)
    if (source_data_root_episode / 'final_data.pkl').exists():
        return preprocess_phystwin(cfg, dataset_root_episode, source_data_root_episode, dt_frame, dx, **kwargs)
    
    # Original PGND format preprocessing
    save = kwargs.get('save', True)
    if save:
        print('Preprocessing episode (Original PGND format):', dataset_root_episode.name)
    else:
        print('Preprocessing episode in-memory (Original PGND format):', source_data_root_episode.name)
    ...
    eef_global_T = np.array([cfg.model.eef_t[0], cfg.model.eef_t[1], cfg.model.eef_t[2]])
    ...
    eef_xyz = eef_xyz.reshape(eef_xyz.shape[0], -1, 3)
    eef_xyz = eef_xyz + eef_global_T

    xyz = np.einsum('nij,jk->nik', xyz, R.T)
    eef_xyz = np.einsum('nij,jk->nik', eef_xyz, R.T)

    if os.path.exists(source_data_root_episode / 'eef_rot.txt'):
        eef_rot = np.loadtxt(source_data_root_episode / 'eef_rot.txt')
        eef_rot = eef_rot.reshape(eef_rot.shape[0], -1, 3, 3)
        eef_rot = R @ eef_rot @ R.T
        eef_rot = torch.from_numpy(eef_rot).reshape(-1, 3, 3)
        eef_quat = kornia.geometry.conversions.rotation_matrix_to_quaternion(eef_rot).numpy().reshape(n_frames, -1, 4)
    else:
        eef_quat = np.zeros((xyz.shape[0], cfg.sim.num_grippers, 4), dtype=np.float32)
        eef_quat[:, :, 0] = 1.0

    if os.path.exists(source_data_root_episode / 'eef_gripper.txt'):
        eef_gripper = np.loadtxt(source_data_root_episode / 'eef_gripper.txt')
        eef_gripper = eef_gripper.reshape(eef_gripper.shape[0], -1)
        assert cfg.sim.num_grippers == eef_gripper.shape[1]
    else:
        eef_gripper = np.zeros((n_frames, cfg.sim.num_grippers), dtype=np.float32)
    ...
    eef_vel = np.zeros_like(eef_xyz)
    eef_vel[:-1] = (eef_xyz[1:] - eef_xyz[:-1]) / dt_frame
    eef_vel[-1] = eef_vel[-2]

    eef_rot_this = kornia.geometry.conversions.quaternion_to_rotation_matrix(torch.from_numpy(eef_quat[:-1]).reshape(-1, 4))
    eef_rot_next = kornia.geometry.conversions.quaternion_to_rotation_matrix(torch.from_numpy(eef_quat[1:]).reshape(-1, 4))
    eef_rot_delta = eef_rot_this.bmm(eef_rot_next.inverse())
    eef_aa = kornia.geometry.conversions.rotation_matrix_to_axis_angle(eef_rot_delta)
...
def fps(x, enabled, n, device, random_start=False):
    ...
    Args:
        x: Points tensor of shape (N, 3)
        enabled: Boolean mask of shape (N,)
        n: Number of points to sample
        device: Device to use
        random_start: Whether to use random starting point (currently ignored)
    
    Returns:
        Indices of sampled points
    """
    assert torch.diff(enabled * 1.0).sum() in [0.0, -1.0]
    # sample_farthest_points expects (B, N, 3) and returns (sampled_points, indices)
    # Note: pytorch3d doesn't support start_idx, so random_start is not used
    enabled_points = x[enabled][None]  # Add batch dimension: (1, N_enabled, 3)
    _, fps_idx = sample_farthest_points(enabled_points, K=n, random_start_point=random_start)
    fps_idx = fps_idx[0]  # Remove batch dimension
    ...
    def __init__(self,
        ...
        with torch.no_grad():
            ...
            for episode_id in range(len(episodes)):
                ...
                episode_info = None # Initialize info for in-memory processing
                
                if self.save_dataset and os.path.exists(dataset_root_episode):
                    ...
                    if len(traj) > 0:
                        ...
                        while overwrite is None:
                            if not sys.stdin.isatty():
                                print(f'Non-interactive environment, defaulting to overwrite for {dataset_root_episode}')
                                overwrite = True
                                break
                            try:
                                ...
                                print(f'EOF detected, defaulting to overwrite for {dataset_root_episode}')
                                overwrite = True
                                break
                        if overwrite == True: 
                            ...
                            states = [torch.load(p, map_location='cpu') for p in traj] # Load existing states
                    elif overwrite is None: overwrite = True
                    elif overwrite == False: continue
                elif overwrite is None: overwrite = True
                elif overwrite == False: continue
                
                if overwrite or not self.save_dataset: # If overwrite is True OR we are processing in-memory
                    if self.save_dataset:
                        dataset_root_episode.mkdir(parents=True, exist_ok=True)
                    
                    ret = preprocess(cfg, dataset_root_episode, source_data_root_episode,
                            self.dt * self.interval / self.skip_frame / self.load_skip_frame, self.dx, save=self.save_dataset)
                    
                    if not self.save_dataset:
                        success, states, info = ret
                        if not success: continue
                        episodes_clean.append(episode_id)
                        traj = [f"memory_{episode_id}_{i}" for i in range(len(states))] # Dummy traj for in-memory
                        episode_info = info
                    else:
                        success = ret
                        if not success:
                        ...
                        episode_info = None # Will load from file later

                if not states: # If states are still empty, it means preprocessing failed or was skipped
                    continue
